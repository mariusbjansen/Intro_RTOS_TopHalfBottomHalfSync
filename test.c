...
for (i=0;i<N_TASKS;++i)
{
    ...
    if (i < N_INTERRUPTS)
    {
        OSTaskCreate(TaskInterrupt, (void *)&TaskData[i], &TaskStk[i][TASK_STK_SIZE-1], i+2);
    }
    else
    {
        OSTaskCreate(Task, (void *)&TaskData[i], &TaskStk[i][TASK_STK_SIZE-1], i+2);
    }
}
...
*/
// Add your code here!!
        // There are 5 semaphors ISRSem[0] to ISRSem[4] corresponding to each interrupt task
        // Each interrupt task knows on which semaphor it shall look by it's function argument
        // character '0' must be subtracted. Refer to function TaskStartCreateTasks
        OSSemPend(ISRSem[(int)((*(char *) pdata) - '0')], 0, &err);
// End of your code!!
        OSSemPend(RandomSem, 0, &err);                            /* Acquire semaphore to perform random numbers        */
        x = rand() % 78;                                          /* Find X position where task number will appear      */
        y = rand() % 15;                                          /* Find Y position where task number will appear      */
